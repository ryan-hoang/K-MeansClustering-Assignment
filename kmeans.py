...
def cluster(vectors, k,centroids):
    ...
    for count in range(vectors.get_shape()[0]):
        x = vectors.getrow(count).toarray()[0]
        dists = []
    ...
    for i in range(vectors.get_shape()[0]): # populate list of clusters with vectors
        ...
        listofclusters[clusterLabels[i]-1].append(vectors.getrow(i).toarray()[0])
...
def initCentroids(vectors, k):
    ...
    for i in range(1,k):
        ...
        blah = vectors.getrow(np.where(cumulativeProbabilities >= rand)[0][0]).toarray()
        #print(type(blah))
